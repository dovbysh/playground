...
function sculprtureUse(itemstack, user, pointed_thing)
    ...
    if pointed_thing.type == "node" and minetest.is_protected(pointed_thing.above, user:get_player_name()) == false then
        ...
        for di = 1, 10 do
            ...
            minetest.set_node(vector.add(pointed_thing.above, { x = di, y = di, z = 0 }), { name = "mapgen_stone" })
            minetest.set_node(vector.add(pointed_thing.above, { x = -di, y = di, z = 0 }), { name = "mapgen_stone" })
            minetest.set_node(vector.add(pointed_thing.above, { x = 0, y = di, z = di }), { name = "mapgen_stone" })
            minetest.set_node(vector.add(pointed_thing.above, { x = 0, y = di, z = -di }), { name = "mapgen_stone" })
        end
    end
    return itemstack
