playground_power={}
playground_item_time=tonumber(minetest.settings:get("item_entity_ttl"))
if not playground_item_time then
	playground_item_time=880
else
	playground_item_time=playground_item_time-20
end

minetest.register_entity("playground:power",{
	hp_max = 100,
	physical = true,
	weight = 0,
	...
	visual = "sprite",
	visual_size = {x=1, y=1},
	textures = {"playground_gravity.png"},
	spritediv = {x=1, y=1},
	is_visible = true,
...
on_activate=function(self, staticdata)
		if playground_power.user then
			self.target=playground_power.target
			self.user=playground_power.user
			self.item=playground_power.item
			playground_power={}
...
on_rightclick=function(self, clicker)
		if clicker:get_player_name()==self.user:get_player_name() then
			...
			if self.item~="gun3" then
				minetest.sound_play("playground_grabnodesend", {pos=pos,max_hear_distance = 7, gain = 1})
			end
			end
		end
		end,
on_step= function(self, dtime)
	...
	if self.throw==0 then
		...
		if enable_playground_throw_stuff_destroys then
			...
			if minetest.registered_nodes[minetest.get_node(vpos).name].walkable and minetest.is_protected(vpos,self.user:get_player_name())==false then
				local obn=playground_spawn_block(vpos)
					...
					if (not ob:get_attach()) and (ob:get_hp()>0 or ob:is_player()) then
						...
						if c~=nil then
							...
							local objpos=ob:get_pos()
								playground_power.item=self.item
								playground_power.user=self.user
								playground_power.target=ob
								local m=minetest.add_entity(objpos, "playground:power")
...
function playground_spawn_block(pos)
	...
	local m=minetest.add_entity(pos, "playground:block")
...
minetest.register_entity("playground:block",{
	hp_max = 30,
	physical = true,
	weight = 0,
...
on_punch=function(self, puncher, time_from_last_punch, tool_capabilities, dir)
		...
		elseif pos~=nil and self.object:get_hp()<=0 and self.drop~="" then
			...
			it:get_luaentity().age=playground_item_time
			return self
		end
	end,
on_activate=function(self, staticdata)
		if not playground_power.new then
			self.object:remove()
			return false
		end
		self.drop=playground_power.drop
		playground_power={}
	end,
on_step= function(self, dtime)
		self.timer=self.timer+dtime
